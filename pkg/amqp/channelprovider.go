...
func (c *pooledChannel) Delivered() bool {
	if c.confirmChan == nil {
		// Delivery confirmation is not enabled. The client should have called DeliveryConfirmationEnabled()
		// to determine whether delivery confirmation was enabled and not call this function. So if a client
		// does call this function then return true so that the client doesn't error out.
		return true
	}
...
func (p *defaultChannelProvider) Channel() (channel, error) {
	...
	return &channelWrapper{Channel: amqpChan, confirmChan: confirmChan}, nil
}
